...
class Conta extends CI_Controller {
    ...
    public function novaconta() {
        if (clienteLogado ()) {			               
			$sessao = $this->session->userdata ( 'loja' );
			$id = $sessao ['codcomprador'];
            redirect ( 'conta/editarconta/'.$id  );
        }
		...
		$data['sexocomprador_M'] = null;
		$data['sexocomprador_F'] = null;
		
		$ufs = array('AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO');		
		foreach($ufs as $ufcomprador){
			$data['BLC_UF'][] = array(
					"UF"			=> $ufcomprador,					
					"sel_estadocivil"		=> null
					);
		}
	...
	public function editarconta($id) {
        $this->title = "Alterar dados da conta";
        $this->showDepartamento = FALSE;
        
        $data = array ();
        $data ['ACAO'] = 'Alterar dados da conta';
        $data ['ACAOFORM'] = site_url ( 'conta/salvar' );		
		
		$res = $this->CompradorM->get ( array (
                "codcomprador" => $id ) , TRUE );
        
        $data ['codcomprador'] = $res->codcomprador;
		$data ['nomecomprador'] = $res->nomecomprador;
        $data ['cpfcomprador'] = $res->cpfcomprador;
        $data ['cepcomprador'] = $res->cepcomprador;
        $data ['enderecocomprador'] = $res->enderecocomprador;
        $data ['cidadecomprador'] = $res->cidadecomprador;
        $data ['emailcomprador'] = $res->emailcomprador;
        $data ['telefonecomprador'] = $res->telefonecomprador;
        $data ['sexocomprador'] = ($res->sexocomprador == 'M')? $data['sexocomprador_M'] = 'checked="checked"' : $data['sexocomprador_F'] = 'checked="checked"';
        $data ['senhacomprador'] = null;
		...
		foreach($ufs as $ufcomprador){
			$data['BLC_UF'][] = array(
					"UF"			=> $ufcomprador,					
					"sel_uf"		=> ($res->ufcomprador == $ufcomprador)?'selected="selected"':null
					);
    ...
    public function salvar() {
       ...
       $this->load->library ( 'form_validation' );
		...
		($codcomprador)? $this->form_validation->set_rules ( 'emailcomprador', 'Email', 'required|valid_email' ) : $this->form_validation->set_rules ( 'emailcomprador', 'Email', 'required|valid_email|is_unique[comprador.emailcomprador]' );
        
        if ($this->form_validation->run () == FALSE) {
            $this->session->set_flashdata ( 'erro', 'Informe os campos corretamente.' . validation_errors () );
            ($codcomprador)? redirect ( 'conta/novaconta/editarconta/'.$codcomprador ) : redirect ( 'conta/novaconta' );
        }
